        ...
        tmp = {
            ...
            'datasetParas':{
                'mosi':{
                    ...
                    'audio_pred_dim': 8,
                    'video_pred_dim': 16,
                    # ---------- loss 总式（与 master 顺序一致）：loss = loss_pred_m + loss_recon_weight*loss_recon + uncertainty_reg_weight*loss_uncertainty_reg + loss_attra_weight*loss_attra ----------
                    # 1) 重建损失
                    'recon_loss': 'SmoothL1Loss',
                    'loss_recon_weight': 1,
                    # 2) 高层对齐损失（SimSiam，与 master 一致便于对比辅助方案）
                    'loss_attra_weight': 1,
                    'use_uncertainty_weighted_attra': True,
                    'use_per_modal_uncertainty_weight': True,
                    'uncertainty_attra_alpha': 2, #`0.2`
                    'modal_attra_weights': {'gmc': 1.0, 'text': 1.1, 'audio': 1.0, 'video': 1.0},
                    # 3) 不确定性正则（与 master 一致，仅方案2 为 fusion_layers+1）
                    'uncertainty_reg_weight': 0.1, 
                    'uncertainty_reg_margin': 1e-5,
                    'uncertainty_before_weight': 0.1,
                    'kl_reg_weight': 1e-3,  # Code_URMF 风格 KL 正则
                    
                    # ---------- 预测头与输出 ----------
                    'post_fusion_dim': 256,
                    'post_fusion_dropout': 0.3, #0.3
                    'H': 3.0,
                    # ---------- 辅助：边界感知 loss（提升 Mult_acc_5/7），0=关闭 ----------
                    'boundary_loss_weight': 0.15,   # 原来0.1
                },
                'mosei':{
                    # the batch_size of each epoch is update_epochs * batch_size
                    'batch_size': 16,
                    ...
                    'use_uncertainty_weighted_attra': True,
                    'use_per_modal_uncertainty_weight': True,
                    'uncertainty_attra_alpha': 2,
                    'modal_attra_weights': {'gmc': 1, 'text': 1.1, 'audio': 1.0, 'video': 1.0},
                    # 3) 不确定性正则（与 master 一致，仅方案2 为 fusion_layers+1）
                    'uncertainty_reg_weight': 0.01,
                    'uncertainty_reg_margin': 1e-4,
                    'uncertainty_before_weight': 0.3,
                    'kl_reg_weight': 1e-3,  # Code_URMF 风格 KL 正则
                ...
                'sims':{
                    ...
                    # 1) 重建损失
                    'recon_loss': 'SmoothL1Loss',
                    'loss_recon_weight': 0.5,
                    # 2) 高层对齐损失（SimSiam，与 master 一致便于对比辅助方案）
                    'loss_attra_weight': 0.7,
                    'use_uncertainty_weighted_attra': True,
                    'use_per_modal_uncertainty_weight': True,
                    'uncertainty_attra_alpha': 0.1,
                    'modal_attra_weights': {'gmc': 1.0, 'text': 1.0, 'audio': 1.0, 'video': 1.0},
                    # 3) 不确定性正则（与 master 一致，仅方案2 为 fusion_layers+1）
                    'uncertainty_reg_weight': 0.1,
                    'uncertainty_reg_margin': 1e-4,
                    'uncertainty_before_weight': 0.3,
